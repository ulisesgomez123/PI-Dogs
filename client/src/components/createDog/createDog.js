...
function handleChangeSelect (e) {
  ...
  return (
    <div>
    <form onSubmit={(e) => dispatchInfo(e,error,input,setError,setInput,temps,dispatch,createDog)} className={style.form}>
    {error.breedError? <span className={style.correct}>this field is OK</span> : <span>it should have only letters</span>}
 <input value={input.breed} name='breed' type='text' onChange={(e)=> validation(e,setInput,setError,error)} placeholder='Breed Name'></input>
 {error.heightError? <span className={style.correct}>this field is OK</span>: <span>it must have this format: min-max </span>}
 <input value={input.height} type='text' name='height' onChange={(e)=> validation(e,setInput,setError,error)} placeholder='Height: (min-max)'></input>
 {error.weightError?<span className={style.correct}>this field is OK</span>: <span>it must have this format: min-max</span>} 
 <input value={input.weight} type='text' name='weight' onChange={(e)=> validation(e,setInput,setError,error)} placeholder='Weight: (min-max)'></input>
 {error.lifeSpanError? <span className={style.correct}>this field is OK</span> : <span>it must have this format: min-max</span>}    
 <input value={input.lifeSpan} type='text' name="lifeSpan" onChange={(e)=> validation(e,setInput,setError,error)} placeholder='Life span: (min-max)'></input>
